...
void countingSortByDigit(int arr[], int n, int exp, int base) {
    ...
    // Build the output array based on the current digit (corrected part)
    int output[n];
    for (int i = n - 1; i >= 0; --i) {
        int digit = (arr[i] / exp) % base;
...
// Recursive Radix Sort function
void recursiveRadixSort(int arr[], int n, int exp, int base) {
    // Base case: if the exponent becomes zero, return
    if (exp == 0) {
        return;
    }

    // Perform counting sort for the current digit place value
    countingSortByDigit(arr, n, exp, base);

    // Recursively sort for the next digit place value
    recursiveRadixSort(arr, n, exp * base, base);
}
...
void radixSort(int arr[], int n, int base) {
    ...
    // Perform recursive radix sort starting from the least significant digit
    recursiveRadixSort(arr, n, 1, base);
}
